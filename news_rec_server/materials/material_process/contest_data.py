...
from conf.proj_path import log_data_path, doc_info_path, user_info_path, tst_data_path
"""
'user_id': 用户id
'article_id': 文章id
'expo_time': 展现时间
...
def load_user_info_data():
    ...
    user_info_df.columns = ['user_id', 'device', 'os', 'province', 'city', 'age','gender']
...
def load_doc_info_data():
    ...
    """   
    doc_info_df = pd.read_csv(doc_info_path, sep='\t')
    doc_info_df.columns = ['article_id', 'title', 'ctime', 'img_num', 'cate','sub_cate', 'key_words']
...
def load_user_behavior_data():
    ...
    # 分块读取，每块50000行
    chunk_size = 50000 
    chunk_iterator = pd.read_csv(log_data_path, sep='\t', chunksize=chunk_size)
    for user_behavior_df in chunk_iterator:
        user_behavior_df.columns = ['user_id', 'article_id', 'expo_time', 'net_status', 'flush_nums', 'expo_position', 'click', 'duration']

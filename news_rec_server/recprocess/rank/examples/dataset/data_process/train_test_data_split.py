...
def main():
    ...
    # 1. 数据读取
    all_data_path = os.path.join(raw_data_path, 'train_data.csv')
    all_data = pd.read_csv(all_data_path, header=None, names=['user_id', 'item_id', '展现时间', '网路环境', '刷新次数', '展现位置', '是否点击', '消费时长（秒）'])

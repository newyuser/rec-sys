...
def main():
    raw_data_path = '../raw_data_sample'
    new_data_path = '../new_data_sample'
    os.makedirs(new_data_path, exist_ok=True)

    # 1. 处理用户文件
    user_path = os.path.join(raw_data_path, 'user_info.csv', header=None)
    user_info = pd.read_csv(user_path)
    ...
    user_info.to_pickle(os.path.join(new_data_path, 'user_info.pkl'))

    # 2. 处理文档文件
    doc_path = os.path.join(raw_data_path, 'doc_info.csv')
    doc_info = pd.read_csv(doc_path, low_memory=False)
